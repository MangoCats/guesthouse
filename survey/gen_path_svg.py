...
def compute_all():
    ...
    pts["U20a"] = outline_geo.fp_pts["F20a"]
...
def render_floorplan(lines, to_svg, pts, outer_poly, inner_poly, inner_segs, layout):
    ...
    # IW3 (west face aligned with IW7, above IW7 L)
    ...
    lines.append(f'<polygon points="{svg}" fill="rgba(160,160,160,0.5)" stroke="#666" stroke-width="0.8"/>')
    # IW9 (old IW3 position, south of IW7 L north face)
    iw9_poly = [(L.iw9.w,L.iw9.s),(L.iw9.e,L.iw9.s),(L.iw9.e,L.iw9.n),(L.iw9.w,L.iw9.n)]
    svg = " ".join(f"{to_svg(*p)[0]:.1f},{to_svg(*p)[1]:.1f}" for p in iw9_poly)
    lines.append(f'<polygon points="{svg}" fill="rgba(160,160,160,0.5)" stroke="#666" stroke-width="0.8"/>')
    # IW10 (horizontal, closet north wall)
    iw10 = L.iw10
    iw10_poly = [(iw10.w,iw10.s),(iw10.e,iw10.s),(iw10.e,iw10.n),(iw10.w,iw10.n)]
    svg = " ".join(f"{to_svg(*p)[0]:.1f},{to_svg(*p)[1]:.1f}" for p in iw10_poly)
    lines.append(f'<polygon points="{svg}" fill="rgba(160,160,160,0.5)" stroke="#666" stroke-width="0.8"/>')
    ...
    svg = " ".join(f"{to_svg(*p)[0]:.1f},{to_svg(*p)[1]:.1f}" for p in iw12_poly)
    lines.append(f'<polygon points="{svg}" fill="rgba(160,160,160,0.5)" stroke="#666" stroke-width="0.8"/>')
    # Appliances
...
def build_outline_cfg(outline_segs, pts, radii):
    ...
    sw = {i: arc_sweep_deg(outline_segs[i], pts)
          for i in [0,2,3,5,7,8,10,11,13,15,17,19,21]}
    return LayerConfig(
        ...
        vertex_styles={
            ...
            "F20a": VertexStyle("F20a", "middle",  0, 13,  "#d32f2f", 1.75, 10),
        ...
        arc_labels={
            ...
            ("F20a","F21"): ArcLabel(f"Arc R={R['R_a20']*12:.0f}\"",
                f"{sw[21]:.1f}\u00b0 CW", "end", -10, -10, 11, "#333"),
        },
        center_marks=[
...
if __name__ == "__main__":
    ...
    _pt_notes = [
        ...
        ("F20a", "line end / arc C20 entry"), ("F21", "arc C20 exit, brg 270\u00b0"),
    ]
    ...
    print(f"  C20:  ({pts['C20'][0]:.4f}, {pts['C20'][1]:.4f})  (F20a->F21 arc center)")
    ...
    _tan_e = _pob_r[0] + 3.0 * _ux_pp  # tangent point 3' from POB
    ...
    centroid_names = [f"F{i}" for i in range(22)] + ["F20a"]
    ...
    print(f"Outline: F0->ArcC0->F1->F2->ArcC2->F3->ArcC3->F4->F5->ArcC5->F6->F7->ArcC7->F8->ArcC8->F9->F10->ArcC10->F11->ArcC11_180->F12->F13->ArcC13->F14->F15->ArcC15->F16->F17->ArcC17->F18->F19->ArcC19->F20->F20a->ArcC20->F21->F0")
