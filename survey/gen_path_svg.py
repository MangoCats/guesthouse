...
def compute_all():
    ...
    for i in [j for j in range(21) if j != 2]:
...
def build_outline_cfg(outline_segs, pts, radii):
    ...
    sw = {i: arc_sweep_deg(outline_segs[i], pts)
          for i in [0,2,4,6,7,9,10,12,14,16,18]}
    return LayerConfig(
        ...
        arc_labels={
            ...
            ("F11","F12"): ArcLabel(f"Arc R={R['R_a11']*12:.0f}\"",
                f"{sw[10]:.1f}\u00b0 CCW", "start", 12, 0, 11, "#333"),
...
if __name__ == "__main__":
    ...
    _pt_notes = [
        ...
        ("F3", "same E as F1"), ("F4", "arc tangent point"),
        ("F5", "arc tangent"),
    ...
    centroid_names = [f"F{i}" for i in range(21) if i != 2]
    ...
    print(f"Outline: F0->ArcC0->F1->F3->ArcC3->F4->F5->ArcC5->F6->F7->ArcC7->F8->ArcC8->F9->F10->ArcC10->F11->ArcC11_180->F12->F13->ArcC13->F14->F15->ArcC15->F16->F17->ArcC17->F18->F19->ArcC19->F20->F0")
