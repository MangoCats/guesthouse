...
Outputs: opts/flat_roof.glb, opts/2in12.glb
"""
...
def _arc_n(sweep_deg):
    return max(4, int(round(abs(sweep_deg) / 3)))
...
def generate(gd=None):
    """Generate both glTF models.  gd is unused (no DB data needed)."""
    os.makedirs(_OPTS, exist_ok=True)
    print("gen_gltf: generating glTF models...")
    _write_model('flat_roof.scad',
                 os.path.join(_OPTS, 'flat_roof.glb'),
                 _build_roof_flat)
    _write_model('2in12.scad',
