...
def _render_furniture(out, data, layout, minik=False, db=False):
    ...
    # 60" diameter circle at SW corner of dresser
    _dr_r = 30.0 / 12.0  # 30" radius
    ...
    # 48" diameter circle at SW corner of dresser
    _dr_r2 = 24.0 / 12.0  # 24" radius
    _dr_sr2 = to_svg(_dr_r2, 0)[0] - to_svg(0, 0)[0]
    out.append(f'<circle cx="{_dr_cx:.1f}" cy="{_dr_cy:.1f}" r="{_dr_sr2:.1f}"'
               f' fill="none" stroke="{APPL_STROKE}" stroke-width="{APPL_SW}"/>')
    ...
    if minik:
        ...
        rk_cy = (layout.iw1_n + _fr_door_s) / 2 + 8.0 / 12.0 + 12.0 / 12.0
        rk_hw = ROCKER_DEPTH / 2   # half E-W (rotated 90°)
        rk_hh = ROCKER_WIDTH / 2   # half N-S (rotated 90°)
        rk_r = ROCKER_CORNER_R
        rk_scx, rk_scy = to_svg(rk_cx, rk_cy)
